    ...
    : mWindow(w)
{
    ...
    mSurface->scale(10.0f, 20.0f, 1.0f);
    mObjects.push_back(mSurface);

    mPlayer = new Player();
    mPlayer->rotate(90, 1.0f, 0.0f, 0.0f);
    mPlayer->rotate(180, 0.0f, 1.0f, 0.0f);
    mPlayer->setPosition(2.5f, 0.5f, 0.0f);
    mObjects.push_back(mPlayer);

    mNPC = new ObjMesh(assetPath + "cube.obj");
    mNPC->setPosition(2.5f, 5.0f, 0.0f);
    mNPC->setName("NPC");
    mObjects.push_back(mNPC);

    mBullet = new ObjMesh(assetPath + "sphere.obj");
    mBullet->setName("Bullet");
    mBullet->visible = false;
    ...
    {
        x += (BOARD_LENGTH / NUMBER_OF_TROPHIES);
        mTrophies[i] = new Trophy();
        mTrophies[i]->setName("Trophy");
        mTrophies[i]->rotate(90, 1.0f, 0.0f, 0.0f);
        mTrophies[i]->setPosition(x, 8.5f, 1.0f);
        mTrophies[i]->setColourIndex(i+1);
        mObjects.push_back(mTrophies[i]);
    }

    //Inital position of the camera
    mCamera.setPosition(QVector3D(-2.5, -5.0, -25));
...
void Renderer::initResources()
{
    ...
    pushConstantRange.size = 17 * sizeof(float);            // 16 floats for the model matrix
    ...
    mTextureHandle = createTexture((assetPath + "orange.jpg")); //Heightmap.jpg HundA.bmp
...
void Renderer::CollisionAndResponse()
{
    for (unsigned int i = 0; i < mObjects.size(); i++)
    {
        for (unsigned int k = 0; k < mObjects.size(); k++)
        {
            // Don't detect collisions against oneself
            if ((mObjects[i]) == (mObjects[k]))
            {
                continue;
            }

            if (mObjects[i]->visible == false || mObjects[k]->visible == false)
            {
                continue;
            }

            float x1 = mObjects[i]->x() + mObjects[i]->dimensions.x(); // first
            float x2 = mObjects[i]->x() - mObjects[i]->dimensions.x();
            float y1 = mObjects[i]->y() + mObjects[i]->dimensions.y();
            float y2 = mObjects[i]->y() - mObjects[i]->dimensions.y();
            float z1 = mObjects[i]->z() + mObjects[i]->dimensions.z();
            float z2 = mObjects[i]->z() - mObjects[i]->dimensions.z();

            float x1_ = mObjects[k]->x() - mObjects[k]->dimensions.x(); // other
            float x2_ = mObjects[k]->x() + mObjects[k]->dimensions.x();
            float y1_ = mObjects[k]->y() - mObjects[k]->dimensions.y();
            float y2_ = mObjects[k]->y() + mObjects[k]->dimensions.y();
            float z1_ = mObjects[k]->z() - mObjects[k]->dimensions.z();
            float z2_ = mObjects[k]->z() + mObjects[k]->dimensions.z();
            ...
            {
                if (mObjects[i]->getName() == "Bullet" && mObjects[k]->getName() == "Trophy")
                {
                    mObjects[i]->visible = false;
                    mObjects[k]->setColourIndex(VisualObject::WHITE);
                }
                else if (mObjects[i]->getName() == "Bullet" && mObjects[k]->getName() == "NPC")
                {
                    mObjects[i]->visible = false;
                }
            }
...
void Renderer::startNextFrame()
{
    if (mVictory == false)
    {
        if ((mInput->A || mInput->LEFT) && mPlayer->x() > 0.5f)
        {
            mPlayer->move(0.1f, 0.0f, 0.0f);
        }
        if ((mInput->D || mInput->RIGHT) && mPlayer->x() < 5.0f)
        {
            mPlayer->move(-0.1f, 0.0f, 0.0f);
        }
        if (mInput->SPACE && mBullet->visible == false)
        {
            mBullet->setPosition(mPlayer->getPosition());
            mBullet->visible = true;
            mBulletsUsed++;
        }

        if (mNPC->isMovingRight)
        {
            mNPC->move(0.1f, 0.0f, 0.0f);
        }
        else if (mNPC->isMovingRight == false)
        {
            mNPC->move(-0.1f, 0.0f, 0.0f);
        }
        ...
        if (mNPC->x() > 5.2f)
        {
            mNPC->isMovingRight = false;
        }
        else if (mNPC->x() < 0.0f)
        {
            mNPC->isMovingRight = true;
        }

        mBullet->move(0.0f, 0.1f, 0.0f);

        if (mBullet->y() > 10.1f) // out of the board range
        {
            mBullet->visible = false;
        }
        ...
        for (int i = 0; i < NUMBER_OF_TROPHIES; i++)
        {
            if (mTrophies[i]->getColourIndex() != VisualObject::WHITE)
            {
                victory = false;
            }
            ...
            if (mTrophies[i]->getColourIndex() == VisualObject::WHITE)
            {
                mTrophies[i]->lifeTime -= 0.1f;
            }

            if (mTrophies[i]->lifeTime < 0.0f)
            {
                mTrophies[i]->lifeTime = mTrophies[i]->initialLifeTime;
                mTrophies[i]->setColourIndex(mTrophies[i]->mInitialColour);
            }
        }

        if (victory && mVictory == false)
        {
            mVictory = true;
            auto end_time = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
            qDebug() << "You won. You used " << mBulletsUsed << " bullets and played for " << duration.count() << " milliseconds.";
        }
    }
    ...
    for (std::vector<VisualObject*>::iterator it=mObjects.begin(); it!=mObjects.end(); it++)
    {
        ...
        setModelMatrix((*it)->getMatrix(), (*it)->getColourIndex()); //mvp);
...
void Renderer::setModelMatrix(QMatrix4x4 modelMatrix, int index)
{
    float* fIndex = new float();
    *fIndex = index;
    ...
    mDeviceFunctions->vkCmdPushConstants(mWindow->currentCommandBuffer(), mPipelineLayout,
                                         VK_SHADER_STAGE_VERTEX_BIT, 16 * sizeof(float), 1 * sizeof(float), fIndex);    //Column-major matrix
